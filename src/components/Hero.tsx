  ...
  return (
    <div className="grid w-screen grid-cols-1 grid-rows-1 gap-20 px-24 mt-5 mb-10 md:grid-cols-2 md:grid-rows-1">
      <div className="flex flex-col items-center w-full md:items-start">
        <div className="mb-4 text-2xl font-semibold text-gblue">
        ...
        <Image
          ...
          className="w-[80%]"
        />
        <div className={`${poppins.className} text-gray-600 text-lg`}>
          A liitle intro about WOW
        </div>
        <div className="w-fit group">
          <a
            href="#regions"
            className="flex items-center p-5 mt-5 rounded-md cursor-pointer h-14 w-fit bg-gblue"
          >
      ...
      <div className="w-screen mb-16 sm:mb-20">
        <div className="w-full h-full flex flex-col md:flex-row">
          ...
          <div className="flex flex-col items-center justify-center mx-24">
              ...
              <button className="flex items-center px-6 py-3 mt-5 text-xl rounded-md cursor-pointer w-fit bg-gblue">
                <span className={`${poppins.className} text-white`}>
                  {currentRegion?.registration ? (
                    <Link href={currentRegion.registration}>
                      Register Now !
                    </Link>
                  ) : (
            ...
            <div
              className={`mb-4 min-[1537px]:mb-8 text-xl sm:text-3xl text-center ${poppins.className}`}
            >
              <div>Date: {currentRegion?.date || "To Be Announced"}</div>
              <div className="mt-2 h-[2px] transition-all bg-gred w-full"></div>
            </div>
            <div
              className={`text-center text-lg sm:text-2xl ${poppins.className}`}
            >
...
const RegionScroll = ({
  ...
  return (
    <>
      <div
        id="region-banner"
        className="relative hidden mt-0 mb-4 md:block min-[1537px]:mt-24"
      >
        <Image
          src="/svg/banner.svg"
          width={1440}
        ...
        <div className="bg-transparent w-fit h-fit">
          ...
          >
            {regionList?.map((region, i) => {
              return (
                <div
                  className="font-[700] transition-all  hover:text-[5rem]  flex flex-col text-center"
                  key={i}
                >
                  <h1
                    style={{ borderLeftWidth: 3 + "px" }}
                    className="border-black border-solid cursor-pointer px-36"
                  >
                    {region.name}
                  </h1>
                  <span className="text-xl text-black border-none">
                    {region.date}
                  </span>
                </div>
              );
...
export default function Hero() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const regionsData: Region[] = await (
          await fetch("/JSON/regions.json")
        ).json();
                ...
                .then((data) => {
                  regionsData.some((region) => {
                    if (
                      ...
                      region.principalSubdivision.toLowerCase() ===
                        data.principalSubdivision.toLowerCase()
                    ) {
                      regionContext.setRegion(region);
                      return true;
